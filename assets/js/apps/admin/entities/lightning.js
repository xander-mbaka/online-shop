define(["app_admin"], function(LightningAbstracts){
  LightningAbstracts.module('Entities', function(Entities, LightningAbstracts, Backbone, Marionette, $, _){

    Backbone.emulateJSON = true;

    Entities.Journal = Backbone.Model.extend({
      urlRoot: "lightning/presentation/journal",
    });

    Entities.JournalCollection = Backbone.Collection.extend({
      url: "/lightning/presentation/journals",
    ...
    Entities.Indicator = Backbone.Model.extend({
      urlRoot: "lightning/presentation/trend",
    });

    Entities.IndicatorCollection = Backbone.Collection.extend({
      url: "/lightning/presentation/trends",
    ...
    Entities.BlogCollection = Backbone.Collection.extend({
      url: "/lightning/presentation/blogs",
      model: Entities.Blog
    });
    ...
    var API = {
      getJournalEntities: function(){
        ...
        $.when(promise).done(function(journals){
          if(journals.length === 0){
            $.get("/lightning/presentation/journals/?journals", function(val){
              if(val.length === 0){
      ...
      getJournalIssues: function(model){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/journals/?issues&journalId=" + journalId, function(val){
      ...
      getIssueArticles: function(model){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/journals/?issue=" + issue + "&journalId=" + journalId, function(val){
              var m = JSON.parse(val);
              //alert(m.length);
              if(m.length === 0){
      ...
      getIndicatorEntities: function(){
        ...
        $.when(promise).done(function(indicators){
          if(indicators.length === 0){
            $.get("/lightning/presentation/trends", function(val){
              if(val.length === 0){
      ...
      getSubscriberEntities: function(){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/admin/?subscribers", function(val){
       ...
       getAdminEntities: function(){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/admin/?admins", function(val){
            var models = initializeModels(JSON.parse(val));
            admins.reset(models);       
          })
      ...
      searchSubscribers: function(name){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/admin/?searchSubscriber&name="+name, function(val){
            var models = initializeModels(JSON.parse(val));
            subscribers.reset(models);       
          })
      ...
      searchAdmins: function(name){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/admin/?searchAdmin&name="+name, function(val){
            var models = initializeModels(JSON.parse(val));
            admins.reset(models);       
          })
      ...
      filterSubscribers: function(key){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/admin/?filterSubscriber&key="+key, function(val){
            var models = initializeModels(JSON.parse(val));
            subscribers.reset(models);       
          })
      ...
      filterAdmins: function(key){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/admin/?filterAdmin&key="+key, function(val){
            var models = initializeModels(JSON.parse(val));
            admins.reset(models);       
          })
       ...
       getCategoryEntities: function(){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/journals/?categories", function(val){
            var models = initializeModels(JSON.parse(val));
            categories.reset(models);       
          })

          /*$.get("/lightning/presentation/journals/?categoricalCounts", function(val){
            var models = initializeModels(JSON.parse(val));
            categories.reset(models);       
          })*/
      ...
      getAccountDetails: function(){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/admin/?adminDetails", function(val){
      ...
      getDashboardEntities: function(){
        ...
        $.when(
          ...
          $.get("/lightning/presentation/admin/?dashDetails", function(val){
    ...
    LightningAbstracts.reqres.setHandler("load:entities", function(){
      API.loadEntities();
      return;
    });

    LightningAbstracts.reqres.setHandler("journal:entities", function(){
      return API.getJournalEntities();
    });

    LightningAbstracts.reqres.setHandler("category:entities", function(){
      return API.getCategoryEntities();
    });

    LightningAbstracts.reqres.setHandler("journal:issues", function(model){
      return API.getJournalIssues(model);
    });

    LightningAbstracts.reqres.setHandler("issue:articles", function(model){
      return API.getIssueArticles(model);
    });

    LightningAbstracts.reqres.setHandler("indicator:entities", function(){
      return API.getIndicatorEntities();
    });

    LightningAbstracts.reqres.setHandler("blog:entities", function(){
      return API.getBlogEntities();
    });

    LightningAbstracts.reqres.setHandler("blog:entity", function(model){
      return API.getBlog(model);
    });

    LightningAbstracts.reqres.setHandler("subscriber:entities", function(){
      return API.getSubscriberEntities();
    });

    LightningAbstracts.reqres.setHandler("subscribers:search", function(name){
      return API.searchSubscribers(name);
    });

    LightningAbstracts.reqres.setHandler("subscribers:filter", function(key){
      return API.filterSubscribers(key);
    });

    LightningAbstracts.reqres.setHandler("admin:entities", function(){
      return API.getAdminEntities();
    });

    LightningAbstracts.reqres.setHandler("admins:search", function(name){
      return API.searchAdmins(name);
    });

    LightningAbstracts.reqres.setHandler("admins:filter", function(key){
      return API.filterAdmins(key);
    });

    LightningAbstracts.reqres.setHandler("account:entities", function(){
      return API.getAccountDetails();
    });

    LightningAbstracts.reqres.setHandler("dashboard:entities", function(){
      return API.getDashboardEntities();
    });

    LightningAbstracts.reqres.setHandler("journal:entity", function(id){
      return API.getJournalEntity(id);
    });

    LightningAbstracts.reqres.setHandler("journal:entity:new", function(id){
